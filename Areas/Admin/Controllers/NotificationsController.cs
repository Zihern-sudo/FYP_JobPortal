// ===============================================
// File: Areas/Admin/Controllers/NotificationsController.cs
// ===============================================
...
namespace JobPortal.Areas.Admin.Controllers
{
    ...
    public class NotificationsController : Controller
    {
        ...
        private int CurrentUserId => this.TryGetUserIdFlexible(out var uid) ? uid : 0;
        ...
        public IActionResult Index(int page = 1, int pageSize = 10, string filter = "all", string? type = null)
        {
            ...
            // Base scope: personal notifications
            IQueryable<notification> q = _db.notifications
                .AsNoTracking()
                .Where(n => n.user_id == uid);

            // Filter: unread
            if (string.Equals(filter, "unread", System.StringComparison.OrdinalIgnoreCase))
                q = q.Where(n => !n.notification_read_status);

            // Filter: type (server-translatable, case-insensitive via collation/LIKE)
            if (!string.IsNullOrWhiteSpace(type))
            {
                var typeExact = type.Trim();
                // Use LIKE without wildcards -> exact match; respects DB collation (typically *_ci => case-insensitive)
                q = q.Where(n => n.notification_type != null && EF.Functions.Like(n.notification_type!, typeExact));
            }
            ...
            // Distinct list of types for the filter bar (scoped to this user)
            ...
            {
                Items = new PagedResult<NotificationListItemViewModel>
                {
                    Items = items,
                    Page = page,
                    PageSize = pageSize,
        ...
        {
            var uid = CurrentUserId;
            if (uid == 0) return Json(new { items = System.Array.Empty<object>() });
